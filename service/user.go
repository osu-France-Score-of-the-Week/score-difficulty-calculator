...
func (s *OsuService) GetUserPinnedScores(id int, token string) ([]models.Score, error) {
	client := http.DefaultClient

	req, err := http.NewRequest("GET", "https://osu.ppy.sh/api/v2/users/"+strconv.Itoa(id)+"/scores/pinned", nil)
	if err != nil {
		return nil, err
	}
